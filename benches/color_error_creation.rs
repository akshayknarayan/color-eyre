...
fn criterion_benchmark(c: &mut Criterion) {
    let (_, hook) = color_eyre::config::HookBuilder::default().into_hooks();
    hook.install().unwrap();
    let mut wrap_err_group = c.benchmark_group("WrapErr");
    wrap_err_group.bench_function("color:wrap_err(eyre!(...))", plain_bench);
    wrap_err_group.bench_function("color:wrap_err_with(|| eyre!(...))", with_bench);
